<script lang="ts">
  ...
  function compareItems(): MenuItem[] {
    ...
    if (others.length === 0) {
      return [
        {
          kind: "item",
          label: "Open another doc to compare",
          disabled: true,
          onSelect: () => {},
        },
      ];
  ...
  async function sortKeys() {
    repairError = "";
    try {
      await doc.sortKeys(true);
    } catch (e) {
      repairError = (e as Error).message;
      setTimeout(() => (repairError = ""), 3000);
    }
  }
</script>
...
<div class="toolbar">
  <div class="group">
    ...
    <button
      ...
      title={compare.pair
        ? "Unlink current diff pair (⌘⇧C)"
        : "Compare with another doc (⌘⇧C)"}
      class="compare-btn"
  ...
  <button
    ...
    aria-haspopup="menu"
    aria-expanded={helpMenu !== null}>?</button
  >
...
{#if repairError}
  <div class="repair-toast err">
    <div class="head">
      ...
      {repairError}
      <button class="close" onclick={clearRepairToast} aria-label="dismiss"
        >×</button
      >
    </div>
    {#if repairChanges.length}
      ...
      <ul>
        {#each repairChanges as c}<li>{c}</li>{/each}
      </ul>
    {/if}
  </div>
{:else if repairOk && repairChanges.length}
  <div class="repair-toast ok">
    <div class="head">
      <strong
        >Repaired ({repairChanges.length} fix{repairChanges.length === 1
          ? ""
          : "es"})</strong
      >
      <button class="close" onclick={clearRepairToast} aria-label="dismiss"
